...
func fetchData() async -> [Ratio] {
    ...
    do {
        for param in Calendar.current.plausibleParameters(from: start) {
            let homepage = try await Plausible.fetch(parameters: param, page: "/")
            let total = try await Plausible.fetch(parameters: param)
            assert(homepage.results.count == total.results.count)
            for (h, t) in zip(homepage.results, total.results) {
                assert(h.date == t.date)
                guard t.visitors > 0 else { continue }
                let ratio = Ratio(date: h.date,
                                  homepageVisitors: h.visitors,
                                  totalVisitors: t.visitors,
                                  ratio: Double(h.visitors)/Double(t.visitors))
...
struct HomepageVisitorRatioChart: View {
    let data: [Ratio]
...
struct Page: View {
    var body: some View {
        VStack(alignment: .leading) {
            ...
            Text("Homepage visitor ratio over time")
                .font(.title.bold())

            HomepageVisitorRatioChart(data: data)
        }
    }
}
...
PlaygroundPage.current.setLiveView(
    Canvas(page: Page(), filename: "homepage-visitor-ratio", width: 600, height: 400)
)
