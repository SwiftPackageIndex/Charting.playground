...
let daysSinceLastRelease = Array(
    Dictionary(grouping: rawData, by: \.packageId)
        .mapValues(VersionHistory.timeSinceLastRelease)
...
struct MaintenanceChart: View {
    var body: some View {
        Chart(binnedData(data: daysSinceLastRelease, desiredCount: 100), id: \.index) { element in
            BarMark(
                x: .value("", element.range),
                y: .value("", element.frequency)
            )
        }
        .chartXAxisLabel("Time since last release / days", position: .bottom, alignment: .center)
...
PlaygroundPage.current.setLiveView(
    Canvas(page: Page(), filename: "last-release", width: 600, height: 600)
)
